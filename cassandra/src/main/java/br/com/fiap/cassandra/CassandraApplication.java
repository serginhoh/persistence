...
public class CassandraApplication {
    public static void main(String[] args) {
        Cluster cluster =
                Cluster.builder().addContactPoints("localhost").withoutMetrics().build();
        Session session = cluster.connect("test");
        CassandraOperations template = new CassandraTemplate(session);
        Pessoa joao = template.insert(new
                Pessoa(UUID.randomUUID().toString(), "Joao Silva", 42));

        System.out.println("\nQuantidade de itens na table: " +
                template.count(Pessoa.class));
